...
func TestWebSearchAliasProxy(t *testing.T) {
	tests := []struct {
		...
		{provider: "tavily", path: "/search", body: `{"query":"xrouter","max_results":1}`},
		{provider: "tavily-search", path: "/search", body: `{"query":"xrouter","max_results":1}`},
		{provider: "serper-search", path: "/search", body: `{"query":"xrouter","max_results":1}`},
		{provider: "exa-search", path: "/search", body: `{"query":"xrouter","max_results":1}`},
	}
	for _, tt := range tests {
		t.Run(tt.provider, func(t *testing.T) {
			...
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.URL.Path != tt.path && r.URL.Path != "/web/search" {
