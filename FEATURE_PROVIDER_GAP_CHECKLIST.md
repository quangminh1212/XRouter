...
| Function/tool calling pass-through | OmniRoute, CLIProxyAPI | Done/Partial | Gemini/Ollama/Anthropic request+response tool-call normalization added; broader provider coverage still partial | Added translator coverage for OpenAI tool declarations to Gemini/Anthropic, Gemini function-call response normalization, and provider-scoped chat-with-tools coverage | P1 |
...
| Provider-scoped compat routes `/api/v1/providers/{provider}/...` | 9router, CLIProxyAPI | Done/Partial | Registered real route alias and regression tests to avoid falling through to root handler; added `/web/fetch`, `/web/search`, `/audio/voices`, `/audio/transcriptions`, `/images/analyze`, `/responses`, and `/messages` alias coverage | P1 |
