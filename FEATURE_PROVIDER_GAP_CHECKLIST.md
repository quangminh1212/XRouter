...
| Provider-scoped compat routes `/api/v1/providers/{provider}/...` | 9router, CLIProxyAPI | Done/Partial | Registered real route alias and regression tests to avoid falling through to root handler; added `/web/fetch`, `/web/search`, `/audio/voices`, `/audio/transcriptions`, `/images/analyze`, and `/responses` alias coverage | P1 |
